...
def worker(jobs, results):
    ...
    model = comsol_interface.Speaker_3D_ComsolInterface('3D_Piezoelectric_Microphone_for_GA_based_Optimization_shida.mph')
    while True:
        try:
...
def boss(solutions):
    ...
    results = Queue()
    processes = []
    workers = 6 # number of workers to hire, the value should be determined by calculating the memory usage for each worker. 
